...
/* A lista de módulos mora em js/modulos.js (compartilhada com admin.html). */
var MODULOS = window.MODULOS;

/* Permissao de modulo por usuario: cada usuario do cofre
   (js/usuarios.js, editado no admin.html) tem "mods", a lista de
   ids que ele enxerga. Sem a lista, ve tudo (cofre antigo).
   É cerca de tela, não segurança: quem manda no que o cofre
   entrega continua sendo a senha. */
function modsDoUsuario() {
  var s = window.Auth && Auth.atual();
  if (!s || typeof COFRE === 'undefined' || !COFRE) return null;
  var lg = String(s.login || '').trim().toLowerCase();
  for (var i = 0; i < COFRE.usuarios.length; i++) {
    if (String(COFRE.usuarios[i].login).toLowerCase() === lg) {
      return COFRE.usuarios[i].mods || null;
    }
  }
  return null;
}
...
function podeVer(m) {
  var mods = modsDoUsuario();
  if (!mods) return true;
  return mods.indexOf(m.id) !== -1;
}
